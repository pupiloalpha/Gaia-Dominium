...
class GameLogic {
  ...
  this.aiTurnStartTime = 0;
  this.MAX_AI_TURN_TIME = 30000;
  this.aiHealthMonitor = null;
  this.aiStuckCount = 0;
  this.MAX_AI_STUCK_COUNT = 3;
...
initializeGame() {
  ...
  // Iniciar monitor de saúde da IA
  this.startAIHealthMonitor();
  ...
  setTimeout(() => {
    console.log('💰 Aplicando renda inicial para:', currentPlayer.name);
    this.applyIncomeForPlayer(currentPlayer);
    
    // Fallback: se após 5 segundos ainda estiver na fase de renda, avançar
    setTimeout(() => {
      if (gameState.currentPhase === 'renda') {
        console.log('⚠️ Fallback: avançando para ações após timeout');
  ...
  }, 1500); // Aumentei o delay inicial para 1.5 segundos
}
  ...
  console.log(`🔄 Avançando fase: ${gameState.currentPhase} -> ${phases[nextIndex]}`);
...
// ==================== SISTEMA DE IA - VERIFICAÇÃO E EXECUÇÃO ====================

shouldIAExecute() {
  const currentPlayer = getCurrentPlayer();
  
  // Verificar se o jogador atual é IA
  const isAI = currentPlayer && (currentPlayer.type === 'ai' || currentPlayer.isAI);
  ...
  // Verificar se não está em progresso
...
startAIHealthMonitor() {
  ...
  this.aiHealthMonitor = setInterval(() => {
    ...
    // SÓ VERIFICAR SE FOR IA E SE ESTIVER EM PROGRESSO
    if (!currentPlayer || !(currentPlayer.type === 'ai' || currentPlayer.isAI)) {
      return;
    }
    
    if (this.aiTurnInProgress) {
      const elapsed = Date.now() - this.aiTurnStartTime;
      
      // Se passou muito tempo, verificar problema
      if (elapsed > 15000) { // 15 segundos máximo
        console.warn('⚠️ IA possivelmente travada há 15 segundos');
        
        // Forçar término limpo
        if (this.shouldAIEndTurn()) {
          console.log('🔄 Forçando término do turno da IA (timeout)');
          this.safeAIEndTurn();
        }
      }
    }
...
async checkAndExecuteAITurn() {
  // Usar a nova validação
  if (!this.shouldIAExecute()) {
    return;
  }
  ...
  try {
    const currentPlayer = getCurrentPlayer();
    console.log(`🤖 Iniciando turno para ${currentPlayer.name} (Fase: ${gameState.currentPhase})`);
    
    // Baseado na fase atual
    switch(gameState.currentPhase) {
      case 'renda':
        console.log('🤖 Fase de renda - avançando...');
        gameState.currentPhase = 'acoes';
        gameState.actionsLeft = GAME_CONFIG.ACTIONS_PER_TURN;
        window.uiManager?.updateUI();
        window.uiManager?.updateFooter();
      ...
      case 'acoes':
        await this.executeAIActions(getAIPlayer(gameState.currentPlayerIndex));
        break;
        
      case 'negociacao':
        await this.executeAINegotiation(getAIPlayer(gameState.currentPlayerIndex));
        break;
    }
...
captureFeedback(message, type) {
  ...
  console.log(`📝 Feedback capturado [${type}]: ${message}`);
  
  // Se for feedback de erro para IA, processar
  const currentPlayer = getCurrentPlayer();
  if (currentPlayer && (currentPlayer.type === 'ai' || currentPlayer.isAI)) {
    if (type === 'error' || type === 'warning') {
      console.log(`🤖 IA recebeu feedback de ${type}: ${message}`);
      this.processAIFeedback(message, type);
    }
  }
}

processAIFeedback(message, type) {
  // Analisar o feedback para tomar decisões
  if (message.includes('não está na fase correta')) {
    console.log('🤖 IA detectou fase incorreta, ajustando...');
    // Se estiver tentando ação na fase errada, avançar fase
    this.advancePhaseForAI();
  }
  
  if (message.includes('Selecione uma região')) {
    console.log('🤖 IA precisa selecionar região primeiro');
    // Marcar que a próxima ação precisa selecionar região
    this.aiNeedsRegionSelection = true;
  }
  
  if (message.includes('Você não controla essa região') || 
      message.includes('Região já tem dono')) {
    console.log('🤖 Região inválida para ação');
    // Remover essa região das oportunidades
    this.invalidateAIRegion();
  }
  
  if (message.includes('Recursos insuficientes')) {
    console.log('🤖 Recursos insuficientes para ação');
    // Pular essa ação e tentar outra
    this.skipCurrentAIAction();
  }
}
...
async completeAITurn() {
  ...
  console.log(`🤖 ${currentPlayer.name} completando turno automaticamente...`);
  
  // Pequeno delay para parecer natural
  await this.delay(1000);
  
  // Forçar término do turno
  this.handleEndTurn();
}

async executeAIActions(ai) {
  const currentPlayer = getCurrentPlayer();
  console.log(`🤖 ${currentPlayer.name} executando ações (${gameState.actionsLeft} restantes)`);
  
  // Executar ações enquanto houver ações disponíveis
  while (gameState.actionsLeft > 0) {
    try {
      // Pequeno delay entre ações para parecer natural
      await this.delay(800);
      
      // Executar uma ação da IA
      await ai.takeTurn(gameState, window.uiManager);
      
      // Atualizar UI após ação
      if (window.uiManager) {
        window.uiManager.updateUI();
        window.uiManager.updateFooter();
      }
  ...
  console.log(`🤖 ${currentPlayer.name} terminou ações`);
...
async executeAINegotiation(ai) {
  const currentPlayer = getCurrentPlayer();
  console.log(`🤖 ${currentPlayer.name} na fase de negociação`);
  ...
  try {
    ...
    if (pending.length > 0) {
      ...
      // Processar cada proposta pendente
      for (const negotiation of pending) {
        await this.delay(1000);
        
        // Avaliar proposta
        const shouldAccept = this.evaluateAINegotiation(ai, negotiation);
        
        if (shouldAccept) {
          console.log(`🤖 ${currentPlayer.name} aceitando proposta`);
          setActiveNegotiation(negotiation);
          await this.delay(500);
          await window.gameLogic.handleNegResponse(true);
        } else {
          console.log(`🤖 ${currentPlayer.name} recusando proposta`);
          setActiveNegotiation(negotiation);
          await this.delay(500);
    ...
    if (gameState.actionsLeft > 0 && currentPlayer.resources.ouro >= 1) {
      console.log(`🤖 ${currentPlayer.name} preparando para enviar proposta`);
...
// Método auxiliar para avaliar negociação (simplificado)
evaluateAINegotiation(ai, negotiation) {
  // Lógica simples de avaliação
  // Em uma implementação completa, isso seria parte da classe AIBrain
  
  const myPlayer = getCurrentPlayer();
  const initiator = gameState.players[negotiation.initiatorId];
  
  if (!myPlayer || !initiator) return false;
  
  // Calcular valor da oferta vs solicitação
  let offerValue = 0;
  let requestValue = 0;
  
  // Avaliar recursos oferecidos
  ...
  // Decisão baseada na personalidade da IA
  const personality = ai.personality.type;
  
  switch(personality) {
    case 'economist':
      // Aceita se oferta >= solicitação
      return offerValue >= requestValue;
      
    case 'diplomat':
      // Mais propenso a aceitar para manter relações
      return offerValue >= requestValue * 0.8;
      
    case 'expansionist':
      // Só aceita se for muito vantajoso
      return offerValue >= requestValue * 1.5;
      
    case 'builder':
      // Aceita se tiver recursos que precisa
      const needsStone = myPlayer.resources.pedra < 5;
      const offerHasStone = negotiation.offer.pedra > 0;
      return (needsStone && offerHasStone) || (offerValue >= requestValue);
...
// Método auxiliar para enviar proposta (simplificado)
async sendAINegotiationProposal(ai) {
  const currentPlayer = getCurrentPlayer();
  
  // Encontrar alvo (outro jogador com ouro)
  const otherPlayers = gameState.players.filter(p => 
    p.id !== currentPlayer.id && p.resources.ouro >= 1
  );
  ...
  // Configurar proposta simples baseada na personalidade
  resetNegotiationState();
  ...
  switch(personality) {
    case 'economist':
      ...
      updateNegotiationResource('offer', 'madeira', 2);
      updateNegotiationResource('request', 'ouro', 1);
      break;
      
    case 'expansionist':
      // Oferece recursos por regiões (simplificado)
      updateNegotiationResource('offer', 'pedra', 3);
      // Não solicita nada específico - apenas testa
      break;
      
    default:
      // Proposta básica
      updateNegotiationResource('offer', 'madeira', 1);
      updateNegotiationResource('request', 'agua', 1);
  }
  
  // Enviar proposta
  if (validateNegotiationState()) {
    await window.gameLogic.handleSendNegotiation();
  }
}

async manageAITurnSafely() {
  const currentPlayer = getCurrentPlayer();
  ...
  console.log(`🤖 Gerenciando turno de ${currentPlayer.name} com segurança`);
  
  // Timer de segurança
  const safetyTimeout = setTimeout(() => {
    console.log('⏰ Timeout de segurança para IA');
    this.forceAIEndTurn();
  }, 30000); // 30 segundos máximo
  
  try {
    // 1. Verificar fase atual e agir adequadamente
    switch(gameState.currentPhase) {
      case 'renda':
        console.log('🤖 IA na fase de renda - avançando...');
      ...
      case 'acoes':
        console.log('🤖 IA na fase de ações');
        // Executar ações normalmente
        await this.checkAndExecuteAITurn();
        break;
        
      case 'negociacao':
        console.log('🤖 IA na fase de negociação');
        // Processar negociação ou pular
        await this.processAINegotiationPhase();
        break;
    }
    
    // 2. Verificar se terminou o turno
    if (gameState.actionsLeft === 0 || this.shouldAIEndTurn()) {
      console.log('🤖 IA deve terminar turno');
      await this.safeAIEndTurn();
    }
    
  } catch (error) {
    console.error('🤖 Erro no gerenciamento seguro:', error);
    await this.forceAIEndTurn();
  } finally {
...
async safeAIEndTurn() {
  console.log('🤖 Terminando turno da IA com segurança');
  ...
  if (recentErrors.length > 2) {
    console.log('⚠️ Muitos erros recentes - forçando término');
    this.forceAIEndTurn();
    return;
  }
  
  // Chamar handleEndTurn normalmente
  if (this.handleEndTurn) {
...
forceAIEndTurn() {
  ...
  console.log(`✅ Turno forçado - agora é ${newPlayer?.name}`);
}

shouldAIEndTurn() {
  const currentPlayer = getCurrentPlayer();
  
  // Se não é IA, não terminar
  if (!currentPlayer || !(currentPlayer.type === 'ai' || currentPlayer.isAI)) {
    return false;
  }
  ...
  // Se tentou muitas ações sem sucesso, terminar
  if (this.feedbackHistory.filter(f => f.type === 'error').length > 5) {
    return true;
  }
...
async handleEndTurn() {
  ...
  // Se for IA, avançar automaticamente
  if (currentPlayer && (currentPlayer.type === 'ai' || currentPlayer.isAI)) {
    console.log(`🤖 ${currentPlayer.name} (IA) processando término de turno`);
    
    // Se estiver na fase de ações com ações restantes, executar ações
    if (gameState.currentPhase === 'acoes' && gameState.actionsLeft > 0) {
      console.log('🤖 IA tem ações pendentes, executando...');
      await this.checkAndExecuteAITurn();
      return;
    }
    
    // Se estiver na fase de negociação, processar negociação
    if (gameState.currentPhase === 'negociacao') {
      console.log('🤖 IA na fase de negociação, processando...');
      await this.checkAndExecuteAITurn();
      return;
    }
    
    // Se terminou todas as fases, avançar para próximo jogador
    console.log('🤖 IA completou todas as fases, avançando turno...');
  }
  ...
  if (gameState.currentPhase === 'negociacao') {
    ...
    gameState.actionsLeft = GAME_CONFIG.ACTIONS_PER_TURN;
    gameState.selectedRegionId = null;
    gameState.currentPhase = 'renda'; // ← VOLTA PARA FASE DE RENDA!
    ...
    // Aplicar renda para o novo jogador
    const newPlayer = getCurrentPlayer();
    console.log('💰 Aplicando renda para novo jogador:', newPlayer.name);
    this.applyIncomeForPlayer(newPlayer);
    
    // Pequeno delay e então verificar se o novo jogador é IA
    setTimeout(() => {
      if (newPlayer.type === 'ai' || newPlayer.isAI) {
        console.log(`🤖 Próximo jogador é IA: ${newPlayer.name}`);
    ...
    // Se estiver na fase de renda, mostrar feedback
    console.log('🔄 Jogador na fase de renda. Aguardando modal...');
    this.showFeedback('Aguarde a fase de renda terminar...', 'info');
    
    // Se o modal não apareceu, tentar forçar
    const incomeModal = document.getElementById('incomeModal');
    if (!incomeModal || incomeModal.classList.contains('hidden')) {
      console.log('⚠️ Modal de renda não visível, tentando mostrar...');
      
      // Verificar se já aplicou renda
      const player = getCurrentPlayer();
      if (player && gameState.currentPhase === 'renda') {
        console.log('💰 Forçando aplicação de renda para:', player.name);
        const bonuses = this.applyIncomeForPlayer(player);
        
        // Se ainda não mostrar, avançar manualmente após timeout
        setTimeout(() => {
          if (gameState.currentPhase === 'renda') {
            console.log('⚠️ Modal não apareceu, avançando manualmente para ações');
            gameState.currentPhase = 'acoes';
            gameState.actionsLeft = GAME_CONFIG.ACTIONS_PER_TURN;
            ...
            this.showFeedback(`${player.name} iniciou a fase de ações.`, 'info');
          }
        }, 3000);
      }
    }
    ...
    console.log('⚠️ Fase não reconhecida:', gameState.currentPhase);
    this.showFeedback('Fase não reconhecida. Contate o suporte.', 'error');
  }
}
  ...
  if (player.id === gameState.currentPlayerIndex && gameState.currentPhase === 'renda') {
    if (player.type === 'ai' || player.isAI) {
      // Para IA, avançar direto para fase de ações
      console.log(`🤖 IA ${player.name} recebeu renda, avançando para ações...`);
      gameState.currentPhase = 'acoes';
      gameState.actionsLeft = GAME_CONFIG.ACTIONS_PER_TURN;
      ...
      // Para jogador humano, mostrar modal normalmente
      ...
      setTimeout(() => {
        if (window.uiManager && window.uiManager.modals && typeof window.uiManager.modals.showIncomeModal === 'function') {
          console.log('✅ Chamando showIncomeModal...');
          window.uiManager.modals.showIncomeModal(player, bonuses);
        } else {
          console.error('❌ showIncomeModal não disponível. uiManager:', !!window.uiManager);
          console.error('❌ uiManager.modals:', window.uiManager?.modals);
          console.error('❌ typeof showIncomeModal:', typeof window.uiManager?.modals?.showIncomeModal);
          
          // Fallback: avançar para ações sem modal
          gameState.currentPhase = 'acoes';
          gameState.actionsLeft = GAME_CONFIG.ACTIONS_PER_TURN;
...
// ==================== SISTEMA DE IA - TURNOS ====================

async checkAndExecuteAITurn() {
  if (this.aiTurnInProgress) return;
  
  const currentPlayer = getCurrentPlayer();
  if (!currentPlayer || !(currentPlayer.type === 'ai' || currentPlayer.isAI)) return;
  
  this.aiTurnInProgress = true;
  
  try {
    const ai = getAIPlayer(gameState.currentPlayerIndex);
    
    if (ai) {
      console.log(`🤖 Executando IA: ${ai.personality.name}`);
      ...
      while (gameState.actionsLeft > 0) {
        ...
        // Se a IA não consumiu ação, sair do loop
        if (gameState.actionsLeft === 0) break;
      }
      
      // Após executar ações, avançar para negociação
      if (gameState.actionsLeft === 0) {
        console.log('🤖 IA completou ações, avançando para negociação');
        setTimeout(() => {
          this.handleEndTurn();
        }, 1000);
      }
...
async handleAITurn() {
  ...
  console.log(`🤖 Gerenciando turno da IA: ${currentPlayer.name} (Fase: ${gameState.currentPhase})`);
  
  // Obtém a instância da IA
  const ai = getAIPlayer(gameState.currentPlayerIndex);
  if (!ai) {
    console.warn(`🤖 Instância de IA não encontrada para jogador ${currentPlayer.name}`);
    this.handleEndTurn();
    return false;
  }
  ...
  try {
    switch(gameState.currentPhase) {
      case 'renda':
        ...
        // Pequeno delay e então executar ações
        await this.delay(1000);
        await this.executeAIActions(ai);
        break;
      ...
      case 'negociacao':
        // Executar negociação da IA
        await this.executeAINegotiation(ai);
        break;
    }
...
async executeAINegotiation(ai) {
  ...
  try {
    // 1. Verificar e responder a propostas pendentes
    const pending = getPendingNegotiationsForPlayer(currentPlayer.id);
    if (pending.length > 0) {
      console.log(`🤖 ${currentPlayer.name} tem ${pending.length} propostas pendentes`);
      await ai.handlePendingNegotiations(pending, gameState);
    }
    
    // 2. Enviar proposta se tiver ouro e ações
    if (gameState.actionsLeft > 0 && currentPlayer.resources.ouro >= 1) {
      console.log(`🤖 ${currentPlayer.name} pode enviar proposta`);
      await ai.sendNegotiationProposal(gameState);
    }
    
    // 3. Se não tiver ouro ou ações, terminar turno
    if (gameState.actionsLeft === 0 || currentPlayer.resources.ouro < 1) {
      console.log(`🤖 ${currentPlayer.name} terminou negociação`);
      await this.delay(1000);
      this.handleEndTurn();
...
showFeedback(message, type = 'info') {
  // Capturar feedback primeiro
  this.captureFeedback(message, type);
  
  // Depois mostrar normalmente
