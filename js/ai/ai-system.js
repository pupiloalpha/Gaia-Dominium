...
class AIBrain {
  ...
  initializeServices(gameLogic) {
    this.negotiationService = new AINegotiationService(this);
    this.actionService = new AIActionService(this, gameLogic);
    this.strategyService = new AIStrategyService(this);
    this.disputeService = new AIDisputeService(gameLogic?.disputeLogic);
  ...
  async _executePhase(gameState, uiManager, analysis) {
    switch (gameState.currentPhase) {
      case 'renda':
        // Renda já foi processada, avançar para ações
  ...
  async _handleIncomePhase(gameState) {
    ...
    if (window.gameLogic?.coordinator) {
  ...
  async _handleActionsPhase(gameState, uiManager, analysis) {
    ...
    if (this.disputeService) {
      const disputeOps = this.disputeService.getDisputeOpportunities(
  ...
  sendNegotiationProposal(gameState) {
    // Alias para createAndSendProposal
